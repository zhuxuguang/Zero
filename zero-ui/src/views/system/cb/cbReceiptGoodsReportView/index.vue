    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:cbReceiptGoodsReportView:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:cbReceiptGoodsReportView:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:cbReceiptGoodsReportView:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:cbReceiptGoodsReportView:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="cbReceiptGoodsReportViewList" @selection-change="handleSelectionChange">
...
<script>
import { listCbReceiptGoodsReportView, getCbReceiptGoodsReportView, delCbReceiptGoodsReportView, addCbReceiptGoodsReportView, updateCbReceiptGoodsReportView } from "@/api/system/cb/cbReceiptGoodsReportView";

export default {
  name: "CbReceiptGoodsReportView",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCbReceiptGoodsReportView(this.queryParams).then(response => {
        this.cbReceiptGoodsReportViewList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加物资领用报表视图";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const type = row.type || this.ids
      getCbReceiptGoodsReportView(type).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改物资领用报表视图";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.type != null) {
            updateCbReceiptGoodsReportView(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addCbReceiptGoodsReportView(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除物资领用报表视图编号为"' + types + '"的数据项？').then(function() {
        return delCbReceiptGoodsReportView(types);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/cbReceiptGoodsReportView/export', {
        ...this.queryParams
      }, `cbReceiptGoodsReportView_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
