    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:cbFixedCostReportView:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:cbFixedCostReportView:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:cbFixedCostReportView:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:cbFixedCostReportView:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="cbFixedCostReportViewList" @selection-change="handleSelectionChange">
...
<script>
import { listCbFixedCostReportView, getCbFixedCostReportView, delCbFixedCostReportView, addCbFixedCostReportView, updateCbFixedCostReportView } from "@/api/system/cb/cbFixedCostReportView";

export default {
  name: "CbFixedCostReportView",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCbFixedCostReportView(this.queryParams).then(response => {
        this.cbFixedCostReportViewList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加固定成本报表视图";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const type = row.type || this.ids
      getCbFixedCostReportView(type).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改固定成本报表视图";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.type != null) {
            updateCbFixedCostReportView(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addCbFixedCostReportView(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除固定成本报表视图编号为"' + types + '"的数据项？').then(function() {
        return delCbFixedCostReportView(types);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/cbFixedCostReportView/export', {
        ...this.queryParams
      }, `cbFixedCostReportView_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
