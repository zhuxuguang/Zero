    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:cbSowBankMonthAvgCostReportView:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:cbSowBankMonthAvgCostReportView:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:cbSowBankMonthAvgCostReportView:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:cbSowBankMonthAvgCostReportView:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="cbSowBankMonthAvgCostReportViewList" @selection-change="handleSelectionChange">
...
<script>
import { listCbSowBankMonthAvgCostReportView, getCbSowBankMonthAvgCostReportView, delCbSowBankMonthAvgCostReportView, addCbSowBankMonthAvgCostReportView, updateCbSowBankMonthAvgCostReportView } from "@/api/system/cb/cbSowBankMonthAvgCostReportView";

export default {
  name: "CbSowBankMonthAvgCostReportView",
  data() {
    return {
      // 遮罩层
      loading: true,
      ...
      total: 0,
      // 母猪存栏月平均成本(元/头)表格数据
      cbSowBankMonthAvgCostReportViewList: [],
      // 弹出层标题
      title: "",
      // 是否显示弹出层
      open: false,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCbSowBankMonthAvgCostReportView(this.queryParams).then(response => {
        this.cbSowBankMonthAvgCostReportViewList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加母猪存栏月平均成本(元/头)";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const type = row.type || this.ids
      getCbSowBankMonthAvgCostReportView(type).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改母猪存栏月平均成本(元/头)";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.type != null) {
            updateCbSowBankMonthAvgCostReportView(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addCbSowBankMonthAvgCostReportView(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除母猪存栏月平均成本(元/头)编号为"' + types + '"的数据项？').then(function() {
        return delCbSowBankMonthAvgCostReportView(types);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/cbSowBankMonthAvgCostReportView/export', {
        ...this.queryParams
      }, `cbSowBankMonthAvgCostReportView_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
