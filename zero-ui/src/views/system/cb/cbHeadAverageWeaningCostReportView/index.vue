    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:cbHeadAverageWeaningCostReportView:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:cbHeadAverageWeaningCostReportView:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:cbHeadAverageWeaningCostReportView:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:cbHeadAverageWeaningCostReportView:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="cbHeadAverageWeaningCostReportViewList" @selection-change="handleSelectionChange">
...
<script>
import { listCbHeadAverageWeaningCostReportView, getCbHeadAverageWeaningCostReportView, delCbHeadAverageWeaningCostReportView, addCbHeadAverageWeaningCostReportView, updateCbHeadAverageWeaningCostReportView } from "@/api/system/cb/cbHeadAverageWeaningCostReportView";

export default {
  name: "CbHeadAverageWeaningCostReportView",
  data() {
    return {
      // 遮罩层
      loading: true,
      ...
      total: 0,
      // 头均断奶成本计算表格数据
      cbHeadAverageWeaningCostReportViewList: [],
      // 弹出层标题
      title: "",
      // 是否显示弹出层
      open: false,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCbHeadAverageWeaningCostReportView(this.queryParams).then(response => {
        this.cbHeadAverageWeaningCostReportViewList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加头均断奶成本计算";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const type = row.type || this.ids
      getCbHeadAverageWeaningCostReportView(type).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改头均断奶成本计算";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.type != null) {
            updateCbHeadAverageWeaningCostReportView(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addCbHeadAverageWeaningCostReportView(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除头均断奶成本计算编号为"' + types + '"的数据项？').then(function() {
        return delCbHeadAverageWeaningCostReportView(types);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/cbHeadAverageWeaningCostReportView/export', {
        ...this.queryParams
      }, `cbHeadAverageWeaningCostReportView_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
