    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:cbFodderView:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:cbFodderView:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:cbFodderView:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:cbFodderView:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="cbFodderViewList" @selection-change="handleSelectionChange">
...
<script>
import { listCbFodderView, getCbFodderView, delCbFodderView, addCbFodderView, updateCbFodderView } from "@/api/system/cb/cbFodderView";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCbFodderView(this.queryParams).then(response => {
        this.cbFodderViewList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getCbFodderView(type).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.type != null) {
            updateCbFodderView(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addCbFodderView(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除饲料详情报表编号为"' + types + '"的数据项？').then(function() {
        return delCbFodderView(types);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/cbFodderView/export', {
        ...this.queryParams
      }, `cbFodderView_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
