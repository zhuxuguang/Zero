    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:cbAvgAdfiCostView:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:cbAvgAdfiCostView:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:cbAvgAdfiCostView:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:cbAvgAdfiCostView:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="cbAvgAdfiCostViewList" @selection-change="handleSelectionChange">
...
<script>
import { listCbAvgAdfiCostView, getCbAvgAdfiCostView, delCbAvgAdfiCostView, addCbAvgAdfiCostView, updateCbAvgAdfiCostView } from "@/api/system/cb/cbAvgAdfiCostView";

export default {
  name: "CbAvgAdfiCostView",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCbAvgAdfiCostView(this.queryParams).then(response => {
        this.cbAvgAdfiCostViewList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加头均日采食量和成本计算视图";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const type = row.type || this.ids
      getCbAvgAdfiCostView(type).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改头均日采食量和成本计算视图";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.type != null) {
            updateCbAvgAdfiCostView(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addCbAvgAdfiCostView(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除头均日采食量和成本计算视图编号为"' + types + '"的数据项？').then(function() {
        return delCbAvgAdfiCostView(types);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/cbAvgAdfiCostView/export', {
        ...this.queryParams
      }, `cbAvgAdfiCostView_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
