    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:cbFabricateReportView:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:cbFabricateReportView:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:cbFabricateReportView:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:cbFabricateReportView:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="cbFabricateReportViewList" @selection-change="handleSelectionChange">
...
<script>
import { listCbFabricateReportView, getCbFabricateReportView, delCbFabricateReportView, addCbFabricateReportView, updateCbFabricateReportView } from "@/api/system/cb/cbFabricateReportView";

export default {
  name: "CbFabricateReportView",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCbFabricateReportView(this.queryParams).then(response => {
        this.cbFabricateReportViewList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加制造费用报表视图";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const type = row.type || this.ids
      getCbFabricateReportView(type).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改制造费用报表视图";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.type != null) {
            updateCbFabricateReportView(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addCbFabricateReportView(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除制造费用报表视图编号为"' + types + '"的数据项？').then(function() {
        return delCbFabricateReportView(types);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/cbFabricateReportView/export', {
        ...this.queryParams
      }, `cbFabricateReportView_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
