    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:cbVaccineVeterinaryReportView:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:cbVaccineVeterinaryReportView:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:cbVaccineVeterinaryReportView:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:cbVaccineVeterinaryReportView:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="cbVaccineVeterinaryReportViewList" @selection-change="handleSelectionChange">
...
<script>
import { listCbVaccineVeterinaryReportView, getCbVaccineVeterinaryReportView, delCbVaccineVeterinaryReportView, addCbVaccineVeterinaryReportView, updateCbVaccineVeterinaryReportView } from "@/api/system/cb/cbVaccineVeterinaryReportView";

export default {
  name: "CbVaccineVeterinaryReportView",
  data() {
    return {
      // 遮罩层
      loading: true,
      ...
      total: 0,
      // 疫苗-兽药报表视图表格数据
      cbVaccineVeterinaryReportViewList: [],
      // 弹出层标题
      title: "",
      // 是否显示弹出层
      open: false,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCbVaccineVeterinaryReportView(this.queryParams).then(response => {
        this.cbVaccineVeterinaryReportViewList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加疫苗-兽药报表视图";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const type = row.type || this.ids
      getCbVaccineVeterinaryReportView(type).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改疫苗-兽药报表视图";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.type != null) {
            updateCbVaccineVeterinaryReportView(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addCbVaccineVeterinaryReportView(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除疫苗-兽药报表视图编号为"' + types + '"的数据项？').then(function() {
        return delCbVaccineVeterinaryReportView(types);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/cbVaccineVeterinaryReportView/export', {
        ...this.queryParams
      }, `cbVaccineVeterinaryReportView_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
